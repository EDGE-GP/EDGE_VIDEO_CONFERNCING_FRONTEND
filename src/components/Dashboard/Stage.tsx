...
const Stage = () => {
  ...
  const { isPending: isJoinMeetingPending, mutateAsync: joinMeeting } =
    useMutation<{
      ...
      onSuccess: (data) => {
        dispatch(meetingActions.setMeeting(data.meeting));
        dispatch(
          meetingActions.setPasswordRequirement(data.passwordRequirment)
        );
        setMeetingRedirectToggle(true);
  ...
  return (
    <>
      {meetingRedirectToggle && (
        <MeetingRedirect
          isOpen={meetingRedirectToggle}
          setIsOpen={setMeetingRedirectToggle}
        />
      )}
      <div className=" h-full card-shadow  pt-4 bg-white md:ml-[17.5rem] ml-auto rounded-3xl  mr-[2rem] ">
        ...
        <h3 className="abel text-[1rem] px-8">
          Edge Stage: Seamlessly Create or Join Meetings for Instant
          Collaboration
        </h3>
        <div className=" relative pb-4 h-[85%]  flex justify-center   items-start">
          <div className="md:w-[40rem] w-[22rem] mt-3">
            <h1 className="abel md:text-[2.5rem] text-[1.8rem] text-center capitalize leading-[3rem]">
              empowering connection, connecting you to the world where every
              hand speaks
            </h1>
